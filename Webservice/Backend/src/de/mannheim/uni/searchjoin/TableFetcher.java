...
public class TableFetcher  {
	...
	public TableData getTableData(String tableName) {
		...
		for (IndexEntry vv : v){
			...
			entries.get(vv.getColumnHeader()).put(vv.getEntryID(), vv.getValue());
		...
		tab.setRelation(relation);
		tab.setKeyColumnIndex(0);
		String keyColumnHeader = null;
		for (IndexEntry vv : v){
			if (vv.isPrimaryKey())
				keyColumnHeader = vv.getColumnHeader();
				for (int columnId = 0; columnId < relation.length; columnId++) {
					if (relation[columnId][0].equals(keyColumnHeader)){
						tab.setKeyColumnIndex(columnId);
					}
				}
		}
