...
//							.fromAnnotatedTable2JSONTableResponse(table.getValue(), table.getKey());
					...
					String[] relTableHeaders = table.getValue().getColumnHeaders();
					List<String> relTableHeadersList = new ArrayList<String>();
					for (String tableHeader: relTableHeaders){
						relTableHeadersList.add(tableHeader.replace("KeyColumn:", ""));
					}
					relTableHeaders = relTableHeadersList.toArray(relTableHeaders);
		

					for (int i = 0; i < relTableHeaders.length; i++) {
						String currentAttribute = StringNormalizer.format(relTableHeaders[i]);
						double confidence = 0;
						...
						if (matcher.getNormalizedTargetSchema().contains(currentAttribute)) {
							...
							tableSchema2TargetSchema.put(Integer.toString(i) + "_" + relTableHeaders[i],
					...
					String filepath1 = "C:/Users/UNI-Mannheim/Documents/Test Scripts/2017-05-15 backend in python/tests/2017-06-06 test java/SubjectsFromQueryTable__" + table.getKey() + ".txt";
					FileWriter writer = new FileWriter(filepath1); 
					for(String columnValue: matcher.getSubjectsFromQueryTable()) {
					  writer.write(columnValue);
					}
					writer.close();
					
					
					String filepath2 = "C:/Users/UNI-Mannheim/Documents/Test Scripts/2017-05-15 backend in python/tests/2017-06-06 test java/SubjectsFromCandidateTable__" + table.getKey() + ".txt";				
					FileWriter writer2 = new FileWriter(filepath2); 
					for(String columnValue2: col) {
					  writer2.write(columnValue2);
					}
					writer2.close();
					...
					for (int i = 0; i < col.length; i++) {
						if (i != table.getValue().getHeaderRowIndex()) {
							...
							if (matcher.getSubjectsFromQueryTable().contains(col[i])) {
								...
								String matched = Integer.toString(matcher.getSubjectsFromQueryTable().indexOf(col[i]) + 1);
					...
					if (!instancesCorrespondences2QueryTable.isEmpty()) {
						...
						System.out.println("**Matched table: " + table.getKey() + relTableHeadersList);
