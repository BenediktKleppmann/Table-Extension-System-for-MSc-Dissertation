...
//			Integer numResults = 5;
		    ...
		    for (int i=0; i<indexReader.maxDoc(); i++) {
			    ...
			    System.out.println("finding blocked partner tables for Table in lucene document " + String.valueOf(i));
				...
				if (i%1000==0){
				     CSVWriter writer = new CSVWriter(new FileWriter("/home/bkleppma/create correspondences/blocked_blocks/first_" + String.valueOf(i) +".csv"), ',');
				     ...
				     for (String tableName1 : blockedTableCombinations.keySet()) {
				    	 ...
				    	 List<String> new_row_list = new ArrayList<String>(correspondingTables);
				    	 new_row_list.add(0, tableName1);
				    	 
				    	 String[] new_row = new_row_list.toArray(new String[new_row_list.size()]);
	...
	public static Processable<Correspondence<MatchableTableRow, MatchableTableColumn>> getInstanceMatches(DataSet<MatchableTableRow, MatchableTableColumn> data1,  DataSet<MatchableTableRow, MatchableTableColumn> data2, File file1, File file2, String correspondenceFolderPath) throws Exception {
		...
		LinearCombinationMatchingRule<MatchableTableRow, MatchableTableColumn> matchingRule = new LinearCombinationMatchingRule<MatchableTableRow, MatchableTableColumn>(0.5);
...
//						private NormalisedDateSimilarity dateSim = new NormalisedDateSimilarity();
						...
						@Override
//						public double compare(MatchableTableColumn a1, MatchableTableColumn a2, Correspondence<MatchableTableRow, Matchable> c) {
						public double compare(MatchableTableColumn a1, MatchableTableColumn a2, SimpleCorrespondence<MatchableTableRow>  c) {
			...
			MatchingEngine<MatchableTableRow, MatchableTableColumn> engine = new MatchingEngine<MatchableTableRow, MatchableTableColumn>();
			// Execute the matching
			Processable<Correspondence<MatchableTableColumn, MatchableTableRow>> correspondences = engine.runDuplicateBasedSchemaMatching(data1.getSchema(), data2.getSchema(), instanceCorrespondences, rule, null, new VotingAggregator<MatchableTableColumn,MatchableTableRow>(true, 0.0008), new NoSchemaBlocker<MatchableTableColumn,MatchableTableRow>());
...
//				String[] schemaCorrespondenceRow = {cor.getFirstRecord().getIdentifier(), cor.getSecondRecord().getIdentifier(), String.valueOf(cor.getSimilarityScore())};
	...
	public static String formatQueryString(String keyColumnString, int maxEditDistance) {
		...
		keyColumnString = keyColumnString.replace(",", " ").replace("  ", " ");
		List<String> keyColumnString_tokens = Arrays.asList(keyColumnString.split(" "));
//		List<String> keyColumnString_tokens = WebTablesStringNormalizer.tokenise(keyColumnString, true);

		System.out.println(keyColumnString);
	    ...
	    keyColumnString = keyColumnString.replace("/","").replace("(","").replace(")","").replace("*","").replace("+","");
