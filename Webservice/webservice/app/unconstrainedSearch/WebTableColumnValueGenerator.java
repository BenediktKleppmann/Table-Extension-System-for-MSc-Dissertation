 ...
 *
 */
public class WebTableColumnValueGenerator extends SchemaValueGenerator<MatchableTableRow, MatchableTableColumn> {
	...
	public void generateBlockingKeys(MatchableTableRow record,
		...
		for(MatchableTableColumn c : record.getSchema()) {
			if(record.hasValue(c)) {
				resultCollector.next(new Pair<String, MatchableTableColumn>(record.get(c.getColumnIndex()).toString(), c));
			}
		}
