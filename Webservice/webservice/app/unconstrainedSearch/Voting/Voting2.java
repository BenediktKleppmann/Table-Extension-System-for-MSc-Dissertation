 ...
 * @param <RecordType>
 */
public class Voting2<ValueType, RecordType extends Matchable & Fusible<SchemaElementType>, SchemaElementType extends Matchable> extends
	...
	public FusedValue<ValueType, RecordType, SchemaElementType> resolveConflict(
		...
		for (FusibleValue<ValueType, RecordType, SchemaElementType> value :values ) {
			...
			if (value.getValue().equals("null") || value.getValue().equals("null") || value.getValue().equals("")){
				frequencies.put(value.getValue(), 0);
			} else {
		...
		for (Map.Entry<ValueType, Integer> entry : frequencies.entrySet()) {
			if (! entry.getKey().equals("null") && ! entry.getKey().equals("nan")){
				if (mostFrequent == null || entry.getValue() > frequencies.get(mostFrequent) ) {
